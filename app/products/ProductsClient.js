    ...
    <div className="productOrderLayout">
      <div className="productCatalog">
        ...
        <div className="productList">{visible.length?visible.map(product=><article className="productCard" key={product.id}>
          ...
          <div className={`productOrderControls ${product.stock_status==="low"?"disabled":""}`}><label><span>수량</span><div className="v21Qty"><button onClick={()=>setQuantities(q=>({...q,[product.id]:Math.max(1,Number(q[product.id]||1)-1)}))}>−</button><input type="number" min="1" value={quantities[product.id]||1} onChange={e=>setQuantities(q=>({...q,[product.id]:e.target.value}))}/><button onClick={()=>setQuantities(q=>({...q,[product.id]:Number(q[product.id]||1)+1}))}>＋</button></div></label><button disabled={product.stock_status==="low"} onClick={()=>addToCart(product)}>{product.stock_status==="low"?"품절":"주문담기"}</button></div>
