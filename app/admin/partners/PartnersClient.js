...
export default function PartnersClient() {
  ...
  const actions=x=><div className="adminActions partnerActions">{gradeSelect(x)}{x.status==="approved"?<><button onClick={()=>change(x.id)}>등급 저장</button><button className="outline" onClick={()=>change(x.id,"pending")}>승인대기로 변경</button></>:<><button onClick={()=>change(x.id,"approved")}>승인</button><button className="outline" onClick={()=>change(x.id,"rejected")}>거절</button></>}</div>;
  return <section className="adminManage contentWidth">
    ...
    <div className="adminToolbar"><div className="adminTabs">{["pending","approved","rejected"].map(s=><button key={s} className={tab===s?"active":""} onClick={()=>setTab(s)}>{labels[s]}</button>)}</div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="회사명 또는 아이디 검색" /></div>
    {message && <div className="adminMessage">{message}</div>}
    <div className="adminTableWrap"><table className="adminTable"><thead><tr><th>신청일</th><th>회사명</th><th>담당자</th><th>아이디</th><th>연락처</th><th>이메일</th><th>상태</th><th>가격등급 / 관리</th></tr></thead><tbody>{filtered.length?filtered.map(x=><tr key={x.id}><td>{x.created_at?new Date(x.created_at).toLocaleDateString("ko-KR"):"-"}</td><td>{x.company_name}</td><td>{x.contact_name}</td><td>{x.username}</td><td>{x.phone}</td><td>{x.email}</td><td><span className={`statusPill ${x.status}`}>{labels[x.status]||x.status}</span></td><td>{actions(x)}</td></tr>):<tr><td colSpan="8" className="emptyRow">해당 거래처가 없습니다.</td></tr>}</tbody></table></div>
