...
export default function MobileNav({ session, loaded, onLogout }) {
  ...
  return (
    <div className="mobileNavWrap">
      <button type="button" className={`hamburger ${open ? "isOpen" : ""}`} aria-label={open ? "메뉴 닫기" : "메뉴 열기"} aria-expanded={open} onClick={() => setOpen((value) => !value)}>
        <span /><span /><span />
      </button>
      ...
      {open && (
        <div className="mobileMenu">
          <div className="mobileMenuInner">
            <button type="button" className="mobileMenuRow mobileCompanyToggle" aria-expanded={companyOpen} onClick={() => setCompanyOpen((value) => !value)}>
              <span>회사소개</span><span className={`mobileChevron ${companyOpen ? "isOpen" : ""}`}>⌄</span>
            </button>
            {companyOpen && <div className="mobileSubmenu">
            ...
            <button type="button" className="mobileMenuRow mobileCompanyToggle" aria-expanded={inquiryOpen} onClick={() => setInquiryOpen((value) => !value)}>
              <span>거래처 신청</span><span className={`mobileChevron ${inquiryOpen ? "isOpen" : ""}`}>⌄</span>
            </button>
            {inquiryOpen && <div className="mobileSubmenu">
